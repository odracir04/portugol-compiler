...
%type <node> WRITE_STATEMENT WRITE_BEGIN WRITE_END WRITE_PARAMS EXTRA_PARAMS EXTRA_PARAM
...
STATEMENT: ASSIGNMENT NEWLINE_  { $$ = createNode(TOKEN, (NodeValue) "STATEMENT", $1, $2); }
            | STATEMENT NEWLINE_  {}
            | IFELSE NEWLINE_ {}
            | SWITCH_STATEMENT NEWLINE_ {}
            | WRITE_STATEMENT NEWLINE_ { $$ = createNode(TOKEN, (NodeValue) "STATEMENT", $1, $2); }
...
WRITE_STATEMENT: WRITE_BEGIN WRITE_END { $$ = createNode(TOKEN, (NodeValue) "WRITE_STATEMENT", $1, $2); };
WRITE_BEGIN: WRITE_ OPEN_CURLY_ { $$ = createNode(TOKEN, (NodeValue) "WRITE_BEGIN", $1, $2); } 
            | WRITE_LINE_ OPEN_CURLY_ { $$ = createNode(TOKEN, (NodeValue) "WRITE_BEGIN", $1, $2); };
WRITE_END: WRITE_PARAMS CLOSE_CURLY_ { $$ = createNode(TOKEN, (NodeValue) "WRITE_END", $1, $2); };
WRITE_PARAMS: VAR_NAME_  { $$ = createNode(TOKEN, (NodeValue) "WRITE_PARAMS", $1, NULL); }
            | STRING_LIT_  { $$ = createNode(TOKEN, (NodeValue) "WRITE_PARAMS", $1, NULL); }
            | VAR_NAME_ EXTRA_PARAMS { $$ = createNode(TOKEN, (NodeValue) "WRITE_PARAMS", $1, $2); }
...
WRITE_: WRITE { $$ = createNode(KEYWORD, (NodeValue) "escreva", NULL, NULL); };
WRITE_LINE_: WRITE_LINE { $$ = createNode(KEYWORD, (NodeValue) "escreval", NULL, NULL); };
READ_: READ {};
AND_: AND {};
NOT_: NOT {};
OR_: OR {};
...
TIMES_: TIMES {};
DIVIDE_: DIVIDE {};
OPEN_CURLY_: OPEN_CURLY { $$ = createNode(KEYWORD, (NodeValue) "(", NULL, NULL); };
CLOSE_CURLY_: CLOSE_CURLY { $$ = createNode(KEYWORD, (NodeValue) ")", NULL, NULL); };
MODULO_: MODULO {};
POWER_: POWER {};
BOOL_LIT_: BOOL_LIT {};

...
start: HEADER VARIABLES ALGORITHM | HEADER ALGORITHM | HEADER FUNCTION_LIST ALGORITHM | HEADER VARIABLES FUNCTION_LIST ALGORITHM;
HEADER: TITLE;
TITLE: ALGORITMO VAR_NAME NEWLINE | TITLE NEWLINE;
VARIABLES: VAR VAR_DECLARATION_LIST;
FUNCTION_LIST: FUNCTION_STATEMENT | PROCEDURE_STATEMENT | FUNCTION_LIST FUNCTION_STATEMENT NEWLINE 
                | FUNCTION_LIST PROCEDURE_STATEMENT NEWLINE | FUNCTION_LIST NEWLINE;
VAR_DECLARATION_LIST: VAR_DECLARATION | VAR_DECLARATION_LIST VAR_DECLARATION;
VAR_DECLARATION: VAR_NAME TYPE_ASSIGNMENT
                | VAR_NAME EXTRA_VARS TYPE_ASSIGNMENT | VAR_DECLARATION NEWLINE;
TYPE_ASSIGNMENT: COLON TYPE NEWLINE;
...
IFELSE: IFBEGIN STATEMENT_LIST IFEND | IFBEGIN STATEMENT_LIST ELSE_BLOCK IFEND;
ELSE_BLOCK: ELSEBEGIN STATEMENT_LIST;
IFBEGIN: IF COMPARISON THEN NEWLINE | IFBEGIN NEWLINE;
...
FOR_BEGIN: FOR VAR_NAME RANGE_BLOCK DO NEWLINE
           | FOR VAR_NAME RANGE_AND_STEP_BLOCK DO NEWLINE;
RANGE_BLOCK:  FROM MATH_EXPRESSION TO MATH_EXPRESSION; 
RANGE_AND_STEP_BLOCK: RANGE_BLOCK STEP MATH_EXPRESSION;         
FOR_END: ENDFOR;
...
PROCEDURE_HEADER: PROCEDURE VAR_NAME NEWLINE
                | PROCEDURE VAR_NAME CALL_ARGUMENTS NEWLINE
                | PROCEDURE VAR_NAME CALL_REFERENCE_ARGUMENTS NEWLINE
                | PROCEDURE_HEADER NEWLINE;
CALL_ARGUMENTS:  OPEN_CURLY PROCEDURE_PARAMS_LIST CLOSE_CURLY;    
CALL_REFERENCE_ARGUMENTS:  OPEN_CURLY VAR PROCEDURE_PARAMS_LIST CLOSE_CURLY;         
...
FUNCTION_CALL: VAR_NAME OPEN_CURLY CLOSE_CURLY
                ...
                | VAR_NAME OPEN_CURLY MULTI_PARAM_CALL CLOSE_CURLY;
MULTI_PARAM_CALL:  MATH_EXPRESSION EXTRA_CALL_PARAMS;               
...
FUNCTION_HEADER: FUNCTION VAR_NAME RETURN_VALUE
                | FUNCTION VAR_NAME CALL_ARGUMENTS RETURN_VALUE
                | FUNCTION VAR_NAME CALL_REFERENCE_ARGUMENTS RETURN_VALUE
                | FUNCTION_HEADER NEWLINE;
RETURN_VALUE: COLON TYPE NEWLINE;                
